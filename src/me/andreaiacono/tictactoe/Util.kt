...
fun minimax(actual: State, depth: Int, player: Player) : Move {
    ...
    if (depth == actual.size * actual.size -1) {
        ...
        lastMove.score = actual.setMove(lastMove).getScore(player)
        return lastMove
    }
    ...
    for (move in moves) {
        ...
        val nextState = actual.setMove(move)
...
fun fromCoordsToMove(move: String, player: Player): Move {
    ...
    // FIXME
    return Move(trimmedMove[0].toInt() - 65, trimmedMove[1].toInt() - 49, player)
}